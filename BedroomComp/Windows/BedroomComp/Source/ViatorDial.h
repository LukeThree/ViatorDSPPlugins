...
public:
    void drawRotarySlider(juce::Graphics &g, int x, int y, int width, int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, juce::Slider &slider) override{
        ...
        juce::Rectangle<float> dialArea (rx, ry, diameter, diameter);
        g.setColour(juce::Colour::fromFloatRGBA(0.05, 0.05, 0.05, 1).withAlpha(0.0f)); //center
        g.fillEllipse(dialArea);

        g.setColour(juce::Colours::black.withAlpha(0.15f).withAlpha(0.0f)); //outline
        g.drawEllipse(rx, ry, diameter, diameter, 10.0f);
        juce::Path dialTick;
        g.setColour(juce::Colours::red.withAlpha(0.5f)); //tick color
        dialTick.addRectangle(0, -radius + 6, 3.0f, radius * 0.4);
        g.fillPath(dialTick, juce::AffineTransform::rotation(angle).translated(centerX, centerY));
    }
};
