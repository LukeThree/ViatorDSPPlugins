...
public:
    void drawRotarySlider(Graphics &g, int x, int y, int width, int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, Slider &slider) override{
        ...
        float diameter = fmin(width, height) * .9;
                ...
                g.setColour(juce::Colour::fromRGB(106, 115, 123)); //center
                g.fillEllipse(dialArea);
                
                g.setColour(juce::Colours::whitesmoke.withAlpha(0.5f)); //outline
                g.drawEllipse(rx, ry, diameter, diameter, 3.0f);
                juce::Path dialTick;
                g.setColour(juce::Colours::whitesmoke.withAlpha(0.5f)); //tick color
